...
```js
// 1. Request a token
const tokenRes = await fetch("http://localhost:3001/api/v1/auth/token", {
  method: "POST",
...
<?php
// 1. Request a token
$ch = curl_init("http://localhost:3001/api/v1/auth/token");
curl_setopt_array($ch, [
    CURLOPT_POST           => true,
    CURLOPT_RETURNTRANSFER => true,
...
The [API Catalog](/catalog) on this site uses **API keys** for simplicity. To try the API:
