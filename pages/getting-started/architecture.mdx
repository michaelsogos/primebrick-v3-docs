...
<Mermaid chart={`graph TB
    ...
    subgraph BE["Backend Layer (System Brick)"]
        Express["Express API<br/>port 3001 · /api/v1/"]
        Auth["Auth · RBAC"]
        Registry["Service Registry"]
        Proxy["Round-Robin Proxy"]
    end

    subgraph MS["Microservice Layer (Docker)"]
    ...
    subgraph Infra["Infrastructure"]
        Casdoor["Casdoor OAuth2<br/>port 8000"]
    ...
    Express --> Registry
    Express --> Proxy
    Express -->|OAuth2| Casdoor
    Express <-->|NATS| NATS
    Proxy -->|HTTP /ws/:code| EmailSender
    Proxy -->|HTTP /ws/:code| MS2
    Proxy -->|HTTP /ws/:code| MS3
    EmailSender <-->|NATS| NATS
    MS2 <-->|NATS| NATS
    Express -->|SQL via DAL| Postgres
...
- **Expose OpenAPI** at `/api/v1/openapi.json`
...
2. Each service exposes its spec at `/api/v1/openapi.json`.
...
This means the [API Catalog](/catalog) always reflects the services that are currently online.

## Next steps

- [Infrastructure](./infrastructure) — detailed infrastructure map with ports and protocols.
